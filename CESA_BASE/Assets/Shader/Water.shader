Shader "Custom/Water"
{
	Properties{
		...
		_EdgeLength("Edge length" , Range(0,50)) = 10
	}
	...
	SubShader{
		...
		CGPROGRAM
			...
			void disp(inout appdata v) {
				fixed waveTex1 = tex2Dlod(_WaveTex1H,
					float4(v.texcoord.xy * _WaveTiling.x + float2(0, _Time.x * _FlowSpeed.x), 0, 0)).a * _Displace.x;
				fixed waveTex2 = tex2Dlod(_WaveTex2H,
					float4(v.texcoord.xy * _WaveTiling.y + float2(0, _Time.x * _FlowSpeed.y), 0, 0)).a * _Displace.y;
