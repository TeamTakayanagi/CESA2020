...
public class UIFuseCreate : MonoBehaviour
{
    ...
    private float m_createCount = AdjustParameter.UI_Object_Constant.CREATE_COOUNT;
    ...
    void Awake()
    {
        if (m_firstCreate > 0)
        {
            ...
            StageCreateMgr.Instance.CreateUIFuse(m_firstCreate, transform, StageCreateMgr.SuffixType.Duplication, StageCreateMgr.SuffixType.Random, true);
        }
    }
    ...
    void Update()
    {
        ...
        {
            m_createCount -= Time.deltaTime * GameMgr.Instance.GameSpeed;
            if (m_createCount <= 0)
            {
